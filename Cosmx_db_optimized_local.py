...
class FromSQL:
    ...
    def __init__(self):
        ...
        self.compiled_transcript, new_slide_transcript_table = self.get_previous_entries('all_slide_transcript_v4')
        new_fov_transcript_table = self.check_for_table('all_fov_transcript_v4')
...
class FindFiles:
    ...
    def __find_files(self):
        ...
        check_re_fun = re.compile(r'complete_code_cell_target_call_coord|CellStatsDir|SpatialBC|_Analysis_Summary.txt|CurrentSpotLists|ExptConfig.txt|perCell_1ChStats.csv|restart_token.json')
...
class LabelFiles:
    ...
    def __init__(self, file_paths):
        ...
        self.paths_df, self.spatial, self.config, self.protein_paths, self.log_paths, self.vote_version = self.label_files(file_paths)
    ...
    def label_files(self, file_paths):
        ...
        for file in file_paths:
            ...
            if 'SpatialBC' in file_name:
                ...
                if slide not in paths_df[run]:
                    paths_df[run][slide] = pd.DataFrame(columns = ['ccc', 'match_eff', 'cell_stats', 'spot_file'])
        ...
        return paths_df, spatial_paths, config_paths, protein_paths_df, log_paths, vote_version

class ParseFile:
    def __init__(self, file_path, fov_label, config = False):
        '''Reads a text file and then searches for regex keywords line by line'''
        print(file_path)
        self.parsed, self.config = self.parse_cellsum_file(file_path = file_path, fov_label = fov_label, config = config)

    def parse_cellsumm_line(self, line, config):
    ...
    def parse_cellsum_file(self, file_path, fov_label, config):
        ...
        with open(file_path, 'r') as file_object:
            ...
            while line:
                ...
                key, match = self.parse_cellsumm_line(line, config)
                if config == False:
                    if key is not None:
                        try:
...
class ScrapeTranscriptStats:
    ...
    def __init__(self, paths, run_label, origin_path, directory_size):
        ...
        self.scrape_spatial_bc(self.timestamp_readable)
        ...
        # Send per fov table to SQL
            for slide in self.master_fov_df:
                fov_tb = ToSQL(self.master_fov_df[slide], 'all_fov_transcript_v4', str(run_label) + '_' + slide, new_table = new_fov_transcript_table, id_columns = ['Run_slide_fov', 'Origin_path'])
        try:
        ...
        if self.profiling_target_calls:
        ...
        slide_tb = ToSQL(self.master_slide_df, 'all_slide_transcript_v4', str(run_label) + '_per_fov_df', new_table = new_slide_transcript_table, id_columns = ['Run_slide', 'Origin_path'])
        try:
        # If new table was created flip the global new table variable
            ...
            for run in self.labeled.paths_df:
                for slide in self.labeled.paths_df[run]:
                    ...
                    for fov in ccc_list.index:
                        try:
                            target_calls_df = pd.read_csv(ccc_list[fov])
                                ...
                                if not self.cellsumm_perslide[slide].empty:
                                    try:
                                        ...
                                        if total_cells > 0:
                                            ...
                                            data_load_df[slide].at[fov, 'Per_cell_{}'.format(key)] = inside_cell_sum /  total_cells
    ...
    def scrape_spatial_bc(self, timestamp_readable):
...
class FindRuns:
    '''Generates a list of runs from a single computer'''
    def __init__(self, input_directory):

        self.run_paths = self.__find_files(input_directory)
    def __find_files(self, input_directory):
...
if __name__ == "__main__":
    ...
    for path in paths.run_paths:
        ...
        ouput_stats = ScrapeTranscriptStats(paths = all_files.paths, run_label = paths.run_paths[path], origin_path = path, directory_size = all_files.directory_size)
